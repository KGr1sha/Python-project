...
# Установка рабочего каталога и определение путей
abspath = os.path.abspath(__file__)
dname = os.path.dirname(abspath)
os.chdir(dname)

SAVE_DIR = os.path.join('..', 'graphs')
DATASET_PATH = os.path.join('..', 'data', 'data/Titanic Dataset.csv')
...
def show_survival_by_gender():
    """
    Функция для отображения количества выживших пассажеров по полу.
    """
...
def show_age_distribution_by_survival():
    """
    Функция для отображения распределения возраста пассажеров по статусу выживания.
    """
    plt.figure(figsize=(12, 8))
    plt.hist([data[data['survived'] == 0]['age'], data[data['survived'] == 1]['age']], bins=10, alpha=0.5,
...
def show_boxplot_fare_by_survival():
    """
    Функция для отображения боксплота распределения стоимости билета по статусу выживания.
    """
...
def show_boxplot_age_by_survival():
    """
    Функция для отображения боксплота распределения возраста по статусу выживания.
    """
...
def show_scatter_fare_age_survived():
    """
    Функция для отображения рассеяного графика стоимости билета и возраста по статусу выживания.
    """
...
def show_scatter_fare_age_pclass():
    """
    Функция для отображения рассеяного графика стоимости билета и возраста, категоризированного по классу пассажера.
    """
    plt.figure(figsize=(12, 8))
